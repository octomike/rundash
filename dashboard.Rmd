...
analysisdata <- reactive({
  ...
  tryCatch({
    d <- rd %>%
      run_add_masks(slopeWindow=input$slopeWindow, hrWindow=input$hrWindow,
                    speedWindow=input$speedWindow, slopeLimit=input$slopeLimit,
                    hrLimit=input$hrLimit, speedLimit=input$speedLimit) %>%
