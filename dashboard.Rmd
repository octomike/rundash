...
rv <- reactiveValues(hrMethod=NULL, hrdpCache=NULL, elevationCache=NULL,
                     mfit=NULL, lmfit=NULL, lmspan=1.0, logs=ring::ring_buffer_env(5),
                     refresh=0)
...
analysisdata <- reactive({
  ...
  if (grepl('SVM', input$modelType)) {
    ...
    rv$lmfit <- svm_lin(d, slopes=input$slopes, hrs=input$hrs, C=0.1)
...
### Model
```{r}
radioButtons('modelType', 'Type of Model', choices = c('Loess/LM', 'SVM Regression'), selected = 'Loess/LM')
```
...
### SVM Params
```{r}
sliderInput('C', value=3, min=-15, max=5, step=1,
            'log2(C) / cost')
sliderInput('sigma', value=-2, min=-15, max=5, step=1,
