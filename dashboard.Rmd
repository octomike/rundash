...
hrBreaks <- reactive({
  ...
  breaks <- thresh * c(0.72, 0.82, 0.87, 0.9, 21)
...
observe({
  ...
  leafletProxy('map') %>%
    ...
    addCircleMarkers(data=data %>% filter(hrzone==hrLabels[1]), fillColor=hrColors[1],
                     stroke=FALSE, radius=4, fillOpacity=1)  %>%
    addCircleMarkers(data=data %>% filter(hrzone==hrLabels[2]), fillColor=hrColors[2],
                     stroke=FALSE, radius=4, fillOpacity=1)  %>%
    addCircleMarkers(data=data %>% filter(hrzone==hrLabels[3]), fillColor=hrColors[3],
                     stroke=FALSE, radius=4, fillOpacity=1)  %>%
    addCircleMarkers(data=data %>% filter(hrzone==hrLabels[4]), fillColor=hrColors[4],
                     stroke=FALSE, radius=4, fillOpacity=1)  %>%
    addCircleMarkers(data=data %>% filter(hrzone==hrLabels[5]), fillColor=hrColors[5],
                     stroke=FALSE, radius=4, fillOpacity=1)
})
```
...
fillCol(
  flex = c(5,7),
  renderDygraph({
    ZonePlot(fitdata, hrBreaks, hrColors, hlCallbackJs)
  }),
  renderDygraph({
...
```{r}
renderPlot({
  HRPacePlot(analysisdata, hrColors, hrBreaks, hrLabels, rv)
})
```
